...
class SampleEventSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $this->command->info('Sample events, tickets, and seat maps seeded ('.count($events).' events).');
    }

    private function seedSeatMap(Event $event): void
    {
        $ticketList = $event->tickets()->orderBy('sort_order')->get();
        $seatedTickets = $ticketList->filter(fn (Ticket $ticket) => $ticket->type === 'vip')->values();

        if ($seatedTickets->isEmpty()) {
            return;
        }

        $seatMap = SeatMap::updateOrCreate(
            ['event_id' => $event->id],
            ['name' => "Denah Kursi {$event->venue}", 'is_published' => true, 'locked_at' => null]
        );
        ...
        $sections = [
            ['name' => 'Gold Barat', 'ticket_id' => $seatedTickets->first()->id, 'rows' => ['A' => 12, 'B' => 12, 'C' => 12]],
        ];

        if ($seatedTickets->count() > 1) {
            $sections[] = ['name' => 'Gold Timur', 'ticket_id' => $seatedTickets->get(1)->id, 'rows' => ['A' => 12, 'B' => 12, 'C' => 12]];
        }
